...
class FolkraceJuht(Node):
    ...
    def _kinemaatiline_limit(self, kiirus: float, poore: float):
        ...
        if valjund > max_lubatud:
            skaala = max_lubatud / valjund
            kiirus *= skaala
            poore *= skaala
        return kiirus, poore
    ...
    def _scan_cb(self, msg: LaserScan):
        ...
        if self._log_counter % 32 == 0:
            self.get_logger().info(
                ...
                f'-> v={kiirus:.2f}m/s  o={poore:.2f}rad/s'
            )
    ...
    def _arvuta_kiirus(self, ranges, ette, vasak, parem):
        ...
        if self._tagurda_streak >= TAGURDA_PUSIVUS:
            ...
            self._olek(f'TAGURDAN (pusiv) -> {"V" if suund > 0 else "P"}')
        ...
        if self._lahe_streak >= LAHE_PUSIVUS and parim_kaugus < 0.4:
            suund = 1.0 if parim_nurk >= 0 else -1.0
            self._olek(f'UMMIK -> pooran {"V" if suund > 0 else "P"}')
            kiirus = 0.0
            poore = suund * POORDEKIIRUS
        elif parim_kaugus < 0.4:
            self._olek(f'ROOMAN (ajutine) -> {parim_nurk:+.0f}o')
            kiirus = ROOMA_KIIRUS
        ...
        self._olek(
            f'PARIM={parim_nurk:+.0f}o d={parim_kaugus:.1f}m '
